content = """{% extends 'base.html' %}
{% block content %}
<h2>Teacher Dashboard</h2>
<p class="text-muted">Welcome, {{ session.full_name }}!</p>
<div class="row mt-3">
  <div class="col-md-3">
    <div class="card text-white bg-info mb-3">
      <div class="card-body">
        <h5>My Papers</h5>
        <p class="display-6">{{ papers|length }}</p>
      </div>
    </div>
  </div>
  <div class="col-md-5">
    <div class="card shadow mb-3">
      <div class="card-body">
        <h5>Create New Paper</h5>
        <form method="POST" action="/teacher/paper/create">
          <div class="mb-2">
            <label class="form-label">Paper Title</label>
            <input type="text" name="title" class="form-control" required placeholder="e.g. Mid-Term Exam 2026">
          </div>
          <div class="mb-2">
            <label class="form-label">Subject</label>
            <select name="subject_id" class="form-select" required>
              {% for s in subjects %}
              <option value="{{ s.id }}">{{ s.name }}</option>
              {% endfor %}
            </select>
          </div>
          <button type="submit" class="btn btn-primary w-100">+ Create Paper</button>
        </form>
      </div>
    </div>
  </div>
</div>
<h4 class="mt-3">My Papers</h4>
<table class="table table-bordered bg-white mt-2">
  <thead class="table-dark">
    <tr><th>Title</th><th>Subject</th><th>Status</th><th>Created</th><th>Actions</th></tr>
  </thead>
  <tbody>
    {% for paper in papers %}
    <tr>
      <td>{{ paper.title }}</td>
      <td>{{ paper.subject.name }}</td>
      <td><span class="badge bg-{{ 'success' if paper.status == 'submitted' else 'secondary' }}">{{ paper.status }}</span></td>
      <td>{{ paper.created_at.strftime('%d-%m-%Y') }}</td>
      <td>
        <a href="{{ url_for('teacher.paper_builder', paper_id=paper.id) }}" class="btn btn-sm btn-warning">Edit</a>
      </td>
    </tr>
    {% else %}
    <tr><td colspan="5" class="text-center">No papers yet. Use the form above!</td></tr>
    {% endfor %}
  </tbody>
</table>
{% endblock %}"""

with open('templates/teacher/dashboard.html', 'w', encoding='utf-8') as f:
    f.write(content)
print("Done!")